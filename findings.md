...
## 失效系统代理与下载终止（2026-07-31）
- 用户截图中的 Windows 手动代理为 `127.0.0.1:10808`；当前注册表 `ProxyEnable=1`、`ProxyServer=127.0.0.1:10808`，当前进程没有 `HTTP_PROXY/HTTPS_PROXY/ALL_PROXY` 环境变量。
- `requests.utils.get_environ_proxies('https://v.douyin.com/')` 当前仍返回 HTTP/HTTPS/FTP 均走 `http://127.0.0.1:10808`，与失败日志的 `ProxyError` 和 `WinError 10061` 完全一致。
- 当前故障发生在抖音页面请求和内置图片下载会话，尚未进入 IDM 媒体投递，因此不是 IDM 配置导致。
- 普通用户路径应保留可用系统代理；只有实际生效代理为 loopback 且端口不可达时，本次任务才临时设置直连绕过，结束后恢复原环境且不改 Windows 代理设置。
- Tkinter 任务使用单个后台线程并在运行时禁用全部输入控件；主按钮可安全切换为取消入口。取消必须由调度层和下载循环主动检查，阻塞中的单次网络请求只能在返回或超时后退出。
- 强制进程代理为不可达的 `127.0.0.1:9` 时，普通 Requests 基线稳定触发 `ProxyError`；新任务上下文记录自动直连提示并取得抖音 HTTP 响应，退出后实际代理映射恢复为 `127.0.0.1:9`。
- 同一强制失效代理下，真实 30 图作品经内置浏览器取得详情，首张图片完整保存并解码为 944×1667；收到终止信号后剩余任务停止，已完成图片与正文保留，测试临时目录最终删除。
- 终止属于协作式取消：候选循环、并发调度和流式字节循环会主动退出并清理不完整目标；单次阻塞网络调用需要等待返回/超时，已异步交给 IDM 的外部任务不会被强制杀进程。
...
## 微信视频号原始质量公开解析（2026-07-31）
- 继续审计 `putyy/res-downloader` 与 `ltaoo/wx_channels_download` 后确认：两者完整客户端仍依赖代理注入来读取电脑微信内部 `objectDesc.media`、`decodeKey` 与 `spec`；但 `wx_channels_download` 另提供了公开分享查询页与 `/api/fetch_video_profile`，只需公开 `sph` 分享链接即可返回当前短期 H.264/H.265 视频地址。
- 这条公开解析入口不是微信官方接口，属于第三方可用性依赖；测试请求只发送用户提供的公开分享链接，不需要 Cookie、微信登录状态、浏览器资料、根证书或本机系统代理。
- 测试作品在线 H.264 为 1024×576、30 FPS、22,105,021 字节、视频约 795 kbps、AAC 约 96 kbps；H.265 为相同分辨率与帧率、7,822,722 字节、视频约 262 kbps、AAC 约 48 kbps。
- 参考项目的“下载原始视频”会从 H.264 地址中只保留 `encfilekey` 与 `token`。同一测试作品这样请求后返回 211,789,061 字节的 MP4，FFprobe 结果为 1920×1080、30 FPS、H.264 约 8.52 Mbps、AAC 约 96 kbps，同时包含视频和音频。
- 因此当前样本的“最高质量”不能从 H.264/H.265 标签或编码效率猜测，也不能直接选择公开 API 首条地址；应优先探测可访问的原始地址，失效时再按可访问在线候选回退，最终以本地 FFprobe 验证为准。
- 原始视频和在线候选均为短期签名 URL，不得写入日志、README、测试夹具或提交；报告只保留公开分享 URL、内容 ID、格式标签、文件大小与本地媒体信息。

## 小红书收藏登录态与页面入口（2026-08-05）
- 小红书旧的 `edith.xiaohongshu.com/api/sns/web/v2/user/me` 在当前浏览器态会返回 HTTP 406、`{"code":-1,"success":false}`；它不能单独用于判断账号是否已登录。
- 旧 GUI 只要看到访客 Cookie 且页面未出现登录文案就显示“账号登录态可用”，但收藏读取依赖该接口取 `userId`，因此随后必然报“登录态不可用”。
- 修复后仅在页面存在当前账号的“我/个人主页” ID 或账号接口真的返回 ID 时才显示账号登录；收藏读取优先从该入口进入“收藏-笔记”并扫描页面，接口分页失败只作为降级。

## 微信贴图的上传原文件、无水印图与水印图分层（2026-07-31）
- 新测试链接 `https://mp.weixin.qq.com/s/1V5RhMNwyVqxNOIl_T1nHg` 是 9 张图片的贴图短链接；渲染后的轮播 DOM 当前只保留水印展示图，不能代表页面提供的全部图片候选。
- 用户截图对应的水印版和页面脚本中的无水印版均为 2560×3840：水印版 1,110,777 字节，无水印版 1,119,233 字节。两者是不同的 qpic CDN 文件，水印已烙进前者像素，不是 `watermark=1` 查询参数动态叠加。
- 每个贴图记录的顶层 `cdn_url` 是无水印展示图，`watermark_info.cdn_url` 是水印展示图；`original_info` 还提供作者上传原文件和声明字节数。
- 截图对应作品的上传原文件实际返回 JPEG 5120×7680、46,308,272 字节，带 ICC 配置且 EXIF Orientation 为 1；因此 2560×3840 并不是该作品的最高质量。
- 真实页面公开响应约 2.3 MB，虽然缺少渲染后的 `#js_content`，但包含 `og:title`、说明、公众号昵称和完整图片记录。可直接补出贴图正文，避免不必要的 Chrome/OpenCLI。
- 旧浏览器脚本尝试回传约 2.5 MB 的整个 `document.documentElement.outerHTML`，在 OpenCLI 1.8.3 的真实调用中 90 秒超时；浏览器兜底应先等待 `#js_content`，再同步返回精简正文结构。
- 完整隔离下载得到 9/9 张 `original_info` 上传原文件、0 失败，总计 246,523,781 字节；实际分辨率依次为 4672×7008、4142×6212、3794×5691、5120×7680、4672×7008、4106×4106、2000×3000、2762×4144、4420×6632。

## 抖音/小红书既有作品最高质量复核（2026-07-31）
- 抖音测试短链解析为图文作品 `7651231218977662821`，共 24 张，不是单视频。用户旧文件实际分辨率为 1962/2160 像素宽、最高 3860 像素高。
- 公开静态页没有作品详情；复用当前 Chrome/OpenCLI 会话后，页面内带登录态的 `/aweme/v1/web/aweme/detail/` 返回完整 24 张记录。每张只提供 3 条无水印 `url_list` 和 3 条水印 `download_url_list`，源尺寸记录与旧文件一致。
- 对 24 张图全部候选实际解码后，当前最佳均为无水印 JPEG；它们与用户旧文件逐张字节数和 SHA-256 完全一致。尝试在同一签名 URL 上移除转换模板或改成 q95/q100、raw、noop，以及替换常见 CDN host，均返回 400/403，不能据此声称存在可下载的更高质量抖音原件。
- 小红书测试作品 `6a620848000000001002508a` 的页面为 3 张 3024×4032 图片和 3 段 Live Photo。旧图为每张约 8.8 MB 的 3024×4032 PNG；Live Photo 为 1080×1440 H.264，字节数与当前最佳候选完全一致。
- 每张小红书图片有 6 条候选：两个 raw host、两个 `format/png` 转换、1080×1440 WebP 展示图和预览图。旧逻辑把 raw 判成不可识别图片，再选择全尺寸 PNG。
- raw 实际响应为 1.43–1.46 MB 的 `image/heic`，`ftyp` 品牌包含 `heic/mif1/tmap`；容器中存在 3024×4032 主图（存储方向 4032×3024）以及更小的辅助 Tile Grid。文件路径为 `note_pre_post_uhdr`，说明它是应优先保留的原始 HDR 容器，而不是损坏响应。
- 修改后的真实隔离下载得到 3 张 3024×4032 HEIC（1,463,160、1,428,935、1,454,005 字节）和原有 3 段 1080×1440 Live Photo。第二次运行 6 个文件全部精确复用，文件数仍为 6；系统临时目录退出后不存在。
## 2026-08-01：抖音收藏夹空结果与 GitHub 更新来源

- 用户在 `C:\Users\Chen\Desktop\新建文件夹` 的正式 EXE 中成功登录抖音并读取 18 个收藏夹，但“图片”收藏夹内容接口返回 HTTP 200、`status_code=0`、空 `aweme_list`。
- 当前 Windows 用户仍启用手动代理 `127.0.0.1:10808`，用户级 HTTP/HTTPS 代理变量也保留同一地址；任务运行器检测端口不可达后已临时设置 `NO_PROXY/no_proxy=*`。这解释代理提示，但不是空作品列表的根因。
- 抖音浏览器返回的同一收藏夹同时包含数字 `collects_id=7655694467143407616` 与字符串 `collects_id_str=7655694467143407401`。前者已超过 JavaScript 安全整数范围并被舍入；网页实际请求使用字符串结尾 `7401`。
- 使用错误结尾 `7616` 请求时返回 0 个作品；使用精确字符串 ID 后，同一登录态、同一路径真实返回 2 个作品，作品 ID 为 `7654041265701222651` 和 `7613246069472334666`。
- 抖音网页会为收藏夹请求附带临时签名并使用 `www-hj.douyin.com`，但本次在修正收藏夹 ID 后，现有普通接口已直接返回完整 2 项，因此无需保存、破解或复用短期签名。
- 仓库最新正式 GitHub Release 为 `v2.1`，Windows 资产名为 `Fusion.Downloader.exe`，API 返回资产大小、浏览器下载地址和 `sha256:` digest。GitHub 官方文档说明公开仓库的 latest release 可以不认证读取，并返回 Release 与 assets 元数据。
- 当前实现版本定为 `2.2.0`；检查结果会识别本地版本高于当前公开 v2.1，不会把旧 Release 当作新版。

## v2.3 Release 资产内部版本不同步（2026-08-06）
