...
def download_note(
    ...
    if note_videos and not cover_only:
    ...
    if download_cover or cover_only:
        try:
            ...
            if cover_only:
                raise XhsDownloadError(f"小红书封面获取失败：{exc}") from exc
...
def build_login_context_script() -> str:
    return (
        ...
        "await sleep(1800);"
        "function profileIdFromUrl(value) { const match = String(value || '').match(/\\/user\\/profile\\/([^/?#]+)/); return match && /^[0-9a-zA-Z_-]{6,64}$/.test(match[1]) ? match[1] : ''; }"
        "function accountIdFromPage() { let id = profileIdFromUrl(location.href); if (id) return id; const ownLink = Array.from(document.querySelectorAll('a[href]')).find(anchor => /^(我|我的|个人主页)$/.test(((anchor.innerText || anchor.textContent) || '').trim())); return ownLink ? profileIdFromUrl(ownLink.href || ownLink.getAttribute('href') || '') : ''; }"
        "let me = null;"
        "try {"
        ...
        "const raw = me && me.data && typeof me.data === 'object' ? me.data : {};"
        "const user = raw.userInfo || raw.user_info || raw.user || raw;"
        "const apiUserId = user.userId || user.user_id || user.id || '';"
        "const userId = String(apiUserId || accountIdFromPage() || '');"
        "const guest = !!user.guest;"
        "const accountDetected = !!userId && !guest;"
        "return {url:location.href, title:document.title || '', cookie:document.cookie || '', loginRequired:!accountDetected, authenticated:accountDetected, accountDetected, userId, me};"
        "})()"
    )
...
def build_collection_list_script() -> str:
    return (
        ...
        "function profileIdFromUrl(url) { const m = String(url || '').match(/\\/user\\/profile\\/([^/?#]+)/); return m && /^[0-9a-zA-Z_-]{6,64}$/.test(m[1]) ? m[1] : ''; }"
        "function currentUserId() { let id = profileIdFromUrl(location.href); if (id) return id; const ownLink = Array.from(document.querySelectorAll('a[href]')).find(anchor => /^(我|我的|个人主页)$/.test(textOf(anchor))); return ownLink ? profileIdFromUrl(ownLink.href || ownLink.getAttribute('href') || '') : ''; }"
        ...
        "function openMyPage() { const entries = Array.from(document.querySelectorAll('a[href],button,div,span')).map(el => ({el,text:textOf(el),rect:visibleRect(el)})).filter(item => item.rect && /^(我|我的|个人主页)$/.test(item.text)).sort((a,b)=>(a.rect.width*a.rect.height)-(b.rect.width*b.rect.height)); if (!entries.length) return false; let node = entries[0].el; for (let d = 0; node && d < 5; d++, node = node.parentElement) if (clickElement(node)) return true; return false; }"
        ...
        "let userId = String(user.userId || user.user_id || user.id || currentUserId() || '');"
        "if (!userId && openMyPage()) { await sleep(1800); userId = currentUserId(); }"
        "let noteCount = '';"
        "if (userId) {"
        ...
        "}"
        "return {url:location.href, title:document.title || '', loginRequired:!userId || !!user.guest, authenticated:!!userId && !user.guest, userId, noteCount, boards, me: meRes.data};"
        "})()"
    )
...
def build_favorite_video_script(limit: int | None = None) -> str:
    ...
    return (
        ...
        "let userId = String(user.userId || user.user_id || user.id || currentUserId() || '');"
        "if (!userId && openMyPage()) { await sleep(1800); userId = currentUserId(); }"
        "if (!userId || user.guest) { const bodyText = (document.body && document.body.innerText || '').slice(0, 3000); return {url:location.href, loginRequired:true, authenticated:false, userId, visibleCount:0, notes, me:meRes.data, bodyText}; }"
        ...
        "const bodyText = (document.body && document.body.innerText || '').slice(0, 3000);"
        "return {url:location.href, title:document.title || '', loginRequired:false, authenticated:true, userId, visibleCount:notes.length, apiPages, notes:maxItems ? notes.slice(0, maxItems) : notes};"
        "})()"
    )
...
def build_collection_notes_script(collection_id: str, limit: int | None = None) -> str:
    ...
    return (
        ...
        "const me = unwrap(meRes.data) || {};"
        "const user = me.userInfo || me.user_info || me.user || me;"
        "function openMyPage() { const entries = Array.from(document.querySelectorAll('a[href],button,div,span')).map(el => ({el,text:textOf(el),rect:visibleRect(el)})).filter(item => item.rect && /^(我|我的|个人主页)$/.test(item.text)).sort((a,b)=>(a.rect.width*a.rect.height)-(b.rect.width*b.rect.height)); if (!entries.length) return false; let node = entries[0].el; for (let d = 0; node && d < 5; d++, node = node.parentElement) { const r = visibleRect(node); if (!r) continue; node.click(); return true; } return false; }"
        "let userId = String(user.userId || user.user_id || user.id || currentUserId() || '');"
        "if (!userId && openMyPage()) { await sleep(1800); userId = currentUserId(); }"
        "const notes = [];"
        "const pages = [];"
        ...
        "let stable = 0; let lastCount = notes.length;"
        "for (let round = 0; round < scrollRounds; round++) { collectDomCards(); if (maxItems && notes.length >= maxItems) break; scrollPage(); await sleep(round < 12 ? 650 : 350); collectDomCards(); if (notes.length === lastCount) stable += 1; else { stable = 0; lastCount = notes.length; } if (!maxItems && stable >= 18 && round > 25) break; }"
        "return {url:location.href, title:document.title || '', loginRequired:!userId || !!user.guest, authenticated:!!userId && !user.guest, userId, collectionId, pages, notes:maxItems ? notes.slice(0, maxItems) : notes};"
        "})()"
    )
