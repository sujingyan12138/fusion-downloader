...
- 小红书作品媒体：单条或批量下载图片、视频、Live Photo。图片会先读取原始对象并识别 HEIC/HEIF/AVIF 容器及主图尺寸；全尺寸原始容器可用时直接保存，以保留 HDR 辅助图和源文件信息，否则再对 PNG/WebP/JPEG 候选逐张实际解码并选择最高质量。
- 小红书评论区图片：复用小红书浏览器登录态读取评论图片并下载原图候选。
- 小红书收藏作品/专辑：登录后可下载“全部收藏作品”，也可像抖音收藏夹一样选择“专辑”批量下载；图片、视频、Live Photo 都会保存。再次运行会重新探测当前最高质量，以便旧 PNG/WebP 升级为后来可访问的原始容器；图片字节完全一致或媒体大小与视频轨验证一致时会复用，不生成重复副本。
...
- 微信公众号文章与贴图：选择“微信”后可粘贴 `mp.weixin.qq.com` 公开文章、`pages/image_detail` 贴图链接及含链接的分享文案，软件会自动识别内容类型。文章与贴图保存为 UTF-8 Markdown；贴图只收集作品画廊图片，不混入头像、二维码和操作引导图。贴图会依次尝试作者上传原文件 `original_info`、无水印 CDN 原图 `cdn_url` 和水印兜底图 `watermark_info`，再按实际解码像素和有效字节数选择最高质量候选写入同级 `assets/`；因此原文件可用时不会停留在网页展示的 2560×3840 版本。普通请求触发微信访问验证时会自动复用当前 Chrome/OpenCLI 会话。
...
- 网络与终止保护：可用的系统代理会照常使用；若 Windows/进程残留的本机代理端口已经关闭，本次任务会自动改用直连并在结束后恢复环境，不会修改系统代理设置。下载运行时主按钮会变为 `终止下载`，可停止后续解析、候选探测和下载；正在阻塞的单次网络请求会在返回或超时后退出。
- 手动检查更新：顶部显示当前版本并提供 `检查更新`。软件只在用户点击时读取本仓库最新的正式 GitHub Release；发现新版后先展示版本号、更新说明和大小，用户同意才下载。下载完成必须同时通过 Release 资产大小和 GitHub SHA-256 校验，用户再次确认后才关闭软件、替换当前 EXE 并重新启动。旧版 EXE 会保存在 `更新备份/`，启动验证失败时自动恢复；`下载器设置.json`、各平台登录态和 `下载结果/` 都不会被覆盖。
...
如果日志显示 `ProxyError` 且地址为 `127.0.0.1`/`localhost`，通常是 Windows 仍保留手动代理设置、但代理程序没有运行，并非 IDM 配置错误。新版会只对这种“本机代理端口不可达”情况临时直连；远程代理、账号权限、HTTP 429、签名过期和地区限制仍按各自原因报告。终止是协作式的：已经完整保存的文件会保留，正在写入的临时/不完整媒体会清理；已经交给外部 IDM 的任务不会被软件强行杀掉。

更新来源固定为本仓库的 [GitHub Releases](https://github.com/sujingyan12138/fusion-downloader/releases)，不是直接下载仓库源码。首次带有软件内更新能力的版本是 v2.2.0，因此更早的版本需要手动下载 v2.2.0 一次；之后只要维护者继续按版本号发布带 SHA-256 的 `Fusion.Downloader.exe`，用户即可通过顶部按钮更新。检查更新失败不会影响下载功能，也不会自动安装来源不明或缺少校验值的文件。
...
小红书原始图片可能保存为 `.heic`、`.heif` 或 `.avif`。这类文件通常比服务端转成的 PNG 小，但能保留原始压缩、HDR 辅助图和容器信息，不应按文件大小误判为低质量。Windows 若无法直接预览 `.heic`，可安装微软 HEIF 图像扩展或使用支持 HEIF 的看图软件；程序不会为了兼容预览而覆盖或转码原始文件。
...
正式发布和更新资产的步骤见 [`RELEASING.md`](./RELEASING.md)。
