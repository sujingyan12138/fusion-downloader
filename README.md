...
- 可选最高质量作品封面：勾选 `同时保存最高质量作品封面` 会在原任务之外额外保存封面；勾选 `仅下载最高质量作品封面` 则只解析和保存封面，不传输视频、图片媒体或评论内容。两种模式均覆盖五个平台的单个、批量与收藏入口，并按实际解码像素、非预览/非水印和有效字节数择优，而不是只相信 URL 名称或声明尺寸。
...
默认输出目录位于软件同级，所有媒体文件和报告直接放在这一层：

```text
下载结果/
```

也可以在界面底部点击“更改位置”选择其他保存位置。程序会在所选位置自动创建 `下载结果/`，并记住该选择；“打开下载结果”始终打开当前使用的文件夹。
