...
- 可选最高质量作品封面：勾选 `同时获取最高质量作品封面` 后，以上五个平台的单作品、评论组合与收藏入口会为每个作品额外保存封面；程序下载并解码候选，以实际像素、非预览/非水印和有效字节数择优，而不是只相信 URL 名称或声明尺寸。
...
文件名会包含平台、作者、标题、作品/笔记 ID 和序号，避免批量下载时覆盖，也减少反复点进多层文件夹。可选封面使用 `_封面_宽x高` 后缀；同一作品已有可解码封面时会复用，不重复生成 `_2`、`_3` 副本。
